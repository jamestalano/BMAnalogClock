...
<p align="center">
<b>BMAnalogClock</b> makes it easy to create and customize clocks for iOS.
</p>
...
## Project Details
Learn more about the BMAnalogClock project requirements, licensing, and contributions.
...
### Sample App
The iOS Sample App included with this project demonstrates how to correctly setup and use BMAnalogClock. You can refer to the sample app for an understanding of how to use and setup BMAnalogClock.

## Documentation
All methods, properties, types, and delegate methods available on the BMAnalogClock class are documented below. If you're using Xcode 5 with BMAnalogClock, documentation is available directly within Xcode (just Option-Click any method for Quick Help).

### Setup
Setting up BMAnalogClock in your project is simple. Follow the steps below to get everything up and running.

 1. Import `"BMAnalogClockView.h"` to the header of your view controller:

         #import "BMAnalogClockView.h"

 2. Implement the `BMAnalogClockDelegate` to the same view controller:

         @interface YourViewController : UIViewController <BMAnalogClockDelegate>

 3.  BMAnalogClockView can be initialized in one of two ways. You can either add it directly to your interface (storyboard file) OR through code. Both ways provide the same initialization, just different ways to do the same thing. Use the method that makes sense for your app or project.

     **Interface Initialization**  
     1 - Add a UIView to your UIViewController  
     2 - Change the class type of the UIView to `BMAnalogClockView`  
     3 - Link the view to your code using an `IBOutlet`. You can set the property to `weak` and `nonatomic`.  
     4 - Select the Connect the `BMAnalogClockView` in your interface. Connect the delegate property to your ViewController.  

     **Code Initialization**  
     Just add the following code to your implementation (usually the `viewDidLoad` method).

         BMAnalogClockView *myClock = [[BMAnalogClockView alloc] initWithFrame:CGRectMake(105, 100, 100, 100)];
...
**By conforming to the `BMAnalogClockDelegate` protocol**  
...
### Reloading the clock
BMAnalogClock has a `reloadClock` method. Calling it will completely reload/reinitialize the clock. `layoutSubviews` will be called again as well as all of the properties. The clock will be entirely redrawn.
...
### Current time
BMAnalogClock makes it possible to set the time on the clock to the time of the user’s device. There are two ways to do so.
...
Here is an example on how to use the method:
    ...
    - (void)currentTimeOnClock:(BMAnalogClockView *)clock Hours:(NSString *)hours Minutes:(NSString *)minutes 
...
Thankfully, BMAnalogClock is highly customizable.

<p align="center"><img src="http://s24.postimg.org/qtdne4ih1/Legende.png"/></p>

#### Customization of the hands
Different properties are here to customize the hands of a clock created with BMAnalogClock.  
...
<b>BMAnalogClock</b> gives you the option to display 12 digits on the clock's face (see above image). To do so, simply set the BOOL property `enableDigit` to YES.
